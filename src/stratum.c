...
#define VERSION_MASK_FULL      UINT32_MAX
...
struct client_info {
    ...
    char        user_agent[SOLO_USER_AGENT_MAX_LEN + 1];
    ...
    uint32_t    version_mask_advertised;
...
/* Preserve job update ordering across async wakeups so fast clean jobs are not overwritten. */
struct pending_job_update {
    struct solo_job             *job;
    bool                        clean_jobs;
    struct pending_job_update   *next;
};
...
static pthread_mutex_t job_update_lock = PTHREAD_MUTEX_INITIALIZER;
static struct pending_job_update *job_update_head;
static struct pending_job_update *job_update_tail;
...
static void reset_retarget_window(struct client_info *info, double now)
{
    if (info == NULL)
        return;
...
static void apply_suggested_difficulty(struct client_info *info, int difficulty)
{
    if (info == NULL)
        return;

    info->difficulty = clamp_difficulty(difficulty);
    info->difficulty_last = info->difficulty;
    reset_retarget_window(info, current_timestamp());
}
...
static void free_pending_job_update(struct pending_job_update *update)
{
    if (update == NULL)
        return;

    solo_job_release(update->job);
    free(update);
}

static void clear_pending_job_updates(void)
{
    struct pending_job_update *update;
    struct pending_job_update *next;

    pthread_mutex_lock(&job_update_lock);
    update = job_update_head;
    job_update_head = NULL;
    job_update_tail = NULL;
    pthread_mutex_unlock(&job_update_lock);

    while (update) {
        next = update->next;
        free_pending_job_update(update);
        update = next;
    }
}

static int queue_job_update(const struct solo_job *job, bool clean_jobs)
{
    struct pending_job_update *update;
    ...
    update->job = solo_job_find(job->job_id);
    if (update->job == NULL) {
        free(update);
        return -1;
    }
    update->clean_jobs = clean_jobs;

    pthread_mutex_lock(&job_update_lock);
    if (job_update_tail)
        job_update_tail->next = update;
    else
        job_update_head = update;
    job_update_tail = update;
    pthread_mutex_unlock(&job_update_lock);
    return 0;
}

static struct pending_job_update *take_pending_job_updates(void)
{
    struct pending_job_update *updates;

    pthread_mutex_lock(&job_update_lock);
    updates = job_update_head;
    job_update_head = NULL;
    job_update_tail = NULL;
    pthread_mutex_unlock(&job_update_lock);
    return updates;
...
static int queue_json_line_locked(struct stratum_client *client, json_t *obj)
{
    char *dumped;
    int rc;
    ...
    dumped = json_dumps(obj, JSON_COMPACT);
    if (dumped == NULL)
        return -1;

    rc = client_queue_send_locked(client, dumped, strlen(dumped));
    free(dumped);
    if (rc < 0)
        return -1;
    return client_queue_send_locked(client, "\n", 1);
...
static int reject_submit_with_code_locked(struct stratum_client *client, json_t *id, int code, const char *reason)
{
    if (client) {
        pthread_mutex_lock(&shares_log_lock);
        global_shares_error++;
    ...
    return send_error_locked(client, id, code, reason);
}
...
static int send_job_locked(struct stratum_client *client, const struct solo_job *job, bool clean_job)
{
    if (ensure_client_coinbase_cache(&client->info, job) < 0)
        return -1;
    if (client->info.version_mask != 0) {
        uint32_t effective_mask = client->info.version_mask & ~job->vbrequired;
        if (effective_mask != client->info.version_mask_advertised) {
            char buf[96];
            int n = snprintf(buf, sizeof(buf),
            ...
            if (client_queue_send_locked(client, buf, (size_t)n) < 0)
                return -1;
            client->info.version_mask_advertised = effective_mask;
        }
    }
...
static int retarget_on_new_share(struct client_info *info)
{
    ...
    double target = (double)settings.target_time;
    ...
    if (new_diff != info->difficulty) {
        ...
        reset_retarget_window(info, now);
        return 1;
    }
    return 0;
}
...
static int handle_configure_locked(struct stratum_client *client, json_t *id, json_t *params)
{
    ...
    client->info.version_mask = 0;
    client->info.version_mask_miner = 0;
    client->info.version_mask_advertised = 0;
    ...
    extensions = json_array_size(params) >= 1 ? json_array_get(params, 0) : NULL;
    options = json_array_size(params) >= 2 ? json_array_get(params, 1) : NULL;
    if (extensions && json_is_array(extensions)) {
        for (size_t i = 0; i < json_array_size(extensions); ++i) {
            ...
            extension_name = json_string_value(item);
            if (strcmp(extension_name, "version-rolling") == 0) {
                uint32_t requested_mask = VERSION_MASK_FULL;
                json_t *mask_obj = options && json_is_object(options)
                    ? json_object_get(options, "version-rolling.mask")
                    : NULL;

                supported = true;
                if (mask_obj != NULL) {
                    if (!json_is_string(mask_obj) ||
                        parse_hex_u32_exact(json_string_value(mask_obj), &requested_mask) < 0) {
                        supported = false;
                    }
                }
                ...
                if (supported) {
                    ...
                    client->info.version_mask_advertised = client->info.version_mask;
                    snprintf(mask_str, sizeof(mask_str), "%08x", client->info.version_mask);
                    if (json_object_set_new(result, "version-rolling.mask", json_string(mask_str)) < 0)
                        goto fail;
                    version_rolling = true;
                }
            }

            if (json_object_set_new(result, extension_name, json_boolean(supported)) < 0)
                goto fail;
        }
    }

    if (id != NULL) {
        if (json_object_set(response, "id", id) < 0)
            goto fail;
    } else if (json_object_set_new(response, "id", json_null()) < 0) {
        goto fail;
    }
    if (json_object_set_new(response, "result", result) < 0)
        goto fail;
    result = NULL;
    if (json_object_set_new(response, "error", json_null()) < 0)
        goto fail;
    if (queue_json_line_locked(client, response) < 0)
        goto fail;
...
static int handle_subscribe_locked(struct stratum_client *client, json_t *id, json_t *params)
{
    ...
    if (json_array_size(params) > 0) {
        ...
        if (json_is_string(user_agent))
            snprintf(client->info.user_agent, sizeof(client->info.user_agent), "%.*s",
                SOLO_USER_AGENT_MAX_LEN, json_string_value(user_agent));
    }
...
static int handle_authorize_locked(struct stratum_client *client, json_t *id, json_t *params)
{
    ...
    if (pass != NULL && strlen(pass) >= 3 && (pass[0] == 'd' || pass[0] == 'D') && pass[1] == '=' && isdigit((unsigned char)pass[2])) {
        apply_suggested_difficulty(&client->info, atoi(pass + 2));
    }
    ...
    const char *extra_nonce2, uint32_t intime, uint32_t inonce, uint32_t version_mask)
{
    ...
    *coinbase_len = stub_len + en2_len + cb2_len;
    ...
    {
        uint32_t effective_mask = info->version_mask & ~job->vbrequired;
        version = (job->version & ~effective_mask) | (version_mask & effective_mask);
    }
...
static int handle_submit_locked(struct stratum_client *client, json_t *id, json_t *params)
{
    ...
    if (job == NULL)
        return reject_submit_with_code_locked(client, id, 21, "Job not found");
    ...
    if (!solo_job_register_share(block_hash)) {
        solo_job_release(job);
        return reject_submit_with_code_locked(client, id, 22, "Duplicate share");
    }

    actual_diff = hash_to_difficulty((unsigned char *)block_hash);
    block_candidate = solo_job_block_hash_valid(job, block_hash);
    ...
    if (!solo_job_share_meets_target(block_hash, client->info.difficulty)) {
        if (!solo_job_share_meets_target(block_hash, client->info.difficulty_last)) {
            if (!block_candidate) {
                solo_job_release(job);
                return reject_submit_with_code_locked(client, id, 23, "Low difficulty share");
            }

            accepted_diff = 0;
            log_info(
                "accept block candidate below share difficulty worker=%s job=%s actual_diff=%.12g assigned_diff=%d",
                client->info.worker,
                job->job_id,
                actual_diff,
                client->info.difficulty
            );
        } else {
...
static int handle_suggest_difficulty_locked(struct stratum_client *client, json_t *id, json_t *params)
{
    ...
    difficulty = (int)request_diff;
    apply_suggested_difficulty(&client->info, difficulty);
...
static void on_accept(struct ev_loop *loop, ev_io *watcher, int revents)
{
    ...
    client->info.last_active_time = current_timestamp();
    client->info.last_share_time = client->info.last_active_time;
    client->info.last_retarget_time = client->info.last_active_time;
...
static void on_job_async(struct ev_loop *loop, ev_async *watcher, int revents)
{
    struct pending_job_update *updates;
    struct pending_job_update *update;
    struct pending_job_update *next_update;
    struct stratum_client *client;
    struct stratum_client *next;
    (void)loop;
    (void)watcher;
    ...
    while ((updates = take_pending_job_updates()) != NULL) {
        update = updates;
        while (update) {
            next_update = update->next;

            pthread_rwlock_wrlock(&clients_lock);
            client = clients_head;
            while (client) {
                next = client->next;
                if (client->info.authorized && send_job_locked(client, update->job, update->clean_jobs) < 0)
...
{
    if (queue_job_update(job, clean_jobs) < 0) {
        log_warn("queue stratum job broadcast fail job=%s", job ? job->job_id : "(null)");
        return;
    }
    if (stratum_loop)
